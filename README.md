...
- **[`merge`](merge/SKILL.md)** — land a PR on a `main` that may have moved: detect it locally
  ...
  check looks like — then ready a draft, merge, and clean up branch, primary clone and worktree,
  each only behind an explicit `state == MERGED` check, since deleting a PR's head branch closes it.
...
may touch, what it may spend, git and PR rules, verification standards, what to do when I push
back). It is tracked here so changes to it
